...
curl -s -X POST http://localhost:8081/records \
  -H "Content-Type: application/json" \
  -d '{"name":"Test Record","description":"For testing"}'
echo -e "\n"

echo "=== 3. GET ALL RECORDS ==="
curl -s -X GET http://localhost:8081/records
echo -e "\n"

echo "=== 4. UPDATE RECORD (ID=1) ==="
curl -s -X PUT http://localhost:8081/records/1 \
  -H "Content-Type: application/json" \
  -d '{"name":"Updated Name","description":"Updated Description"}'
echo -e "\n"

echo "=== 5. DELETE RECORD (ID=1) ==="
curl -s -X DELETE http://localhost:8081/records/1
echo -e "\n"
